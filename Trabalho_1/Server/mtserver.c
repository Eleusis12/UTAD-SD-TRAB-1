...
DWORD WINAPI handleconnection(LPVOID lpParam)
{
	...
	while (TRUE) {
		...
		if (strcmp(strRec, "date") == 0) {
			...
			if (!key_generated) {
				printf("ERROR: N�o foi poss�vel gerar a chave do euromilh�es");
			}
...
// Fun��o que gera uma chave do euromilhoes de forma aleat�ria
int* random_key(int (*random_number)(int, int)) {
...
/*Retorna um random n�mero inteiro, no intervalo [min_num, max_num]*/
int random_number(int min_num, int max_num)
{
